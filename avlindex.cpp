...
using namespace std;
/**
 * @brief avlIndex::avlIndex : simply takes in item for adding extra files
 * @param h
 */
avlIndex::avlIndex(string h) : look{h}
{}
/**
 * @brief avlIndex::getTF: this takes a reader/ reads all data
 * and then we use that data to compute TF scores from all of our data
 */
void avlIndex::getTF() {
    reads.stopWords();
    reads.readsTagsNames();
    //if we are setting look as -1, go through normal questions, otherwise add files
    (look == "-1") ? reads.readQuestionNames(0) : reads.addQ(look);
    int counter =0;
...
/**
 * @brief avlIndex::join : this method takes both our IDF items and our TF items and merges them together
            ...
            if(occurs[reads.IDs[counter]].contains(n)) {
                if(occurs[reads.IDs[counter]][n] >= 10) {
                    if(tgID.contains(n)) {  //reading Id's associated w/tags
                        ...
                        pair<int, double> par;
                        par.second = occurs[reads.IDs[counter]][n]; //this is the TFidf score
                        par.second *= 2;
                        par.first = reads.IDs[counter];
                        fileC[n].push_back(par);
                    }
...
/**
 * @brief avlIndex::writeFile : Now we are actually writing the index interface file\n
 * we use our final items and format the data using TFIDF and ID# for easy searching from user perspective
 */
void avlIndex::writeFile() {
    //this actually writes the index interface file
    //finding top 50 words
    sort(top50.begin(), top50.end(), []( const pair<string,int> &p1,
    ...
    write << reads.totalQs << " " <<reads.count << " " <<reads.totalsize << endl;
    for(int i =0; i < 50; i++) {
        write << top50[i].first << " ";
    }
    write << endl;
...
/**
 * @brief avlIndex::evaluate : This is where we are loading the data into our Index file\n
 ...
 */
void avlIndex::evaluate(string qur) {
    ...
    //paginate printing so we have to read everything for it to happen
    if(rests.empty())
...
/**
 * @brief avlIndex::getRests: this returns the top 15 values so the user interface
 * has limited interaction with many other items in the program
 * @return : return the top 15 result ids
 */
vector<int> avlIndex::getRests() {
    return rests;
}
/**
 * @brief avlIndex::adjustStats : this adjusts the top 15 stats by refrence\n
 ...
 */
void avlIndex::adjustStats(int& id, int &qs, string &top) {
    id = totalQ;
    qs = totalWord;
    top = tops50;
}
