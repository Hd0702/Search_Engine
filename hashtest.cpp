...
using namespace std;
/**
 * @brief TEST_CASE : this is the Hash_map catch session, it tests all the same methods as the AVLtree
 * so we can use them interchangebly
 */
TEST_CASE("Hash_map ","[hash_map]") {
    hash_map<string,int> test1;
    hash_map<string,vector<int>> test2;
    test2.insert("test");
    test2.insert("test1");
    SECTION("[] Operator") {
        for(int i =0; i < 50; i ++) {
    ...
    hash_map<int,string> tst;
    for(int i =0; i < 50; i ++) {
        tst.insert(i);
        tst[i] = "test";
    }
    SECTION("COPY CONSTRUCTOR") {
        hash_map<int,string> tok(tst);
        REQUIRE(tok[0] == "test");
        REQUIRE(tok[49] == "test");
        hash_map<string,vector<int>> tst2(test2);
    ...
    SECTION("IS_EMPTY") {
        ...
        hash_map<int,hash_map<int,int>> top;
        top.insert(2);
